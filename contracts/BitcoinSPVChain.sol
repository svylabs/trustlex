...
contract BitcoinSPVChain is ERC20, ISPVChain {
  ...
  constructor(bytes memory initialConfirmedBlockHeader, uint256 height, uint256 _initialEpochTime) {
      BlockHeader memory header = this.parseBytesToBlockHeader(initialConfirmedBlockHeader);
  ...
  /**
     ...
     output: 0x04030201
   */
  function leToUint32(bytes calldata bz, uint startIndex) pure public returns (uint32 result) {
  ...
  /**
     ...
     output: 0x04030201
   */
  function leToBytes4(bytes calldata bz, uint startIndex) pure public returns (bytes4 result) {
  ...
  function leToBytes32(bytes calldata bz, uint startIndex) pure public returns (bytes32 result) {
  ...
  function nBitsToTarget(bytes4 nBits) pure public returns (uint256 target) {
  ...
  // If block is valid accept the block and store it.
     ...
     */
  function validateBlock(BlockHeader memory header, BlockHeader memory previousBlock) public view returns (bool result) {
    uint256 target = nBitsToTarget(header.nBits);
    ...
    if (header.blockHeight % 2016 == 1) {
       ...
       uint256 newTarget = retargetAlgorithm(nBitsToTarget(previousBlock.nBits), epochStartBlockTime, previousBlock.time);
       require(newTarget == nBitsToTarget(header.nBits));
    }
    return true;
  }
  ...
  function confirmBlock(BlockHeader memory header) private {
  ...
  function submitBlock(bytes calldata blockHeader) external {
    BlockHeader memory header = parseBytesToBlockHeader(blockHeader);
    bytes32 previousHeaderHash = leToBytes32(blockHeader, 4);
    ...
    require(validateBlock(header, previousBlock));
